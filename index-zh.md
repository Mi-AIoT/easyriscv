...
尽管本教程内容很长，但仍然做了一些简化。以下是与现实世界中“真实”的 RISC-V 架构以及“真实”的 RISC-V 汇编代码相比，最显著的一些简化和遗漏：

- 这里的汇编语法与 LLVM 汇编器和 GNU Binutils 用于 RISC-V 的语法类似。但是，它们并不完全相同。
- 伪指令和 CSR（控制与状态寄存器）的数量远比我所描述的要多。
- `li` 伪指令应该支持更广泛的常数范围。
- `mstatus` 寄存器的复杂程度远超我的描述。
- `%hi`、`%lo`、`%pcrel_hi`、`%pcrel_lo` 等操作也比我所描述的更为复杂。

此外，还有一些在 RISC-V 世界中非常重要或随处可见的主题，但我选择不予讨论：

- 64 位架构
- 压缩指令（Compressed instructions）
- 其他特权架构和操作系统主题：中断（Interrupts）、内存保护（Memory protection）、虚拟内存（Virtual memory）等
